...
async function existingRunTrackCount(ownerId, runId) {
  const rows = await rest(
    `/rest/v1/pacetune_tracks?owner_id=eq.${encodeURIComponent(ownerId)}&run_id=eq.${encodeURIComponent(String(runId))}&select=track_id`
  );
...
export async function persistReport(ownerId, report, source = "live") {
  ...
  for (const run of runs) {
    const existingTracks = await existingRunTrackCount(ownerId, run.run_id);
    const incomingTracks = run.tracks_during_run || [];
    const shouldPreserveExistingTracks = existingTracks > incomingTracks.length;
    ...
    if (!shouldPreserveExistingTracks) {
      await rest(
    ...
    const trackRows = incomingTracks.map((track) => ({
      owner_id: ownerId,
      run_id: String(run.run_id),
    ...
    if (trackRows.length && !shouldPreserveExistingTracks) {
      await rest("/rest/v1/pacetune_tracks?on_conflict=owner_id,run_id,track_id,ended_at", {
        method: "POST",
        body: trackRows,
        prefer: "resolution=merge-duplicates,return=minimal"
      });
    ...
    if (splitRows.length && !shouldPreserveExistingTracks) {
      await rest("/rest/v1/pacetune_splits?on_conflict=owner_id,run_id,split_index", {
        method: "POST",
        body: splitRows,
        prefer: "resolution=merge-duplicates,return=minimal"
      });
    ...
    if (splitTrackRows.length && !shouldPreserveExistingTracks) {
      await rest("/rest/v1/pacetune_split_tracks?on_conflict=owner_id,run_id,split_index,track_id,ended_at", {
        method: "POST",
        body: splitTrackRows,
        prefer: "resolution=merge-duplicates,return=minimal"
      });
