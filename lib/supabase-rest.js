...
async function hasExistingRunTracks(ownerId, runId) {
  const rows = await rest(
    `/rest/v1/pacetune_tracks?owner_id=eq.${encodeURIComponent(ownerId)}&run_id=eq.${encodeURIComponent(String(runId))}&select=track_id&limit=1`
  );
...
export async function persistReport(ownerId, report, source = "live") {
  ...
  for (const run of runs) {
    const existingTracks = await hasExistingRunTracks(ownerId, run.run_id);
    const incomingTracks = run.tracks_during_run || [];
    const shouldPreserveExistingTracks = existingTracks && incomingTracks.length === 0;
