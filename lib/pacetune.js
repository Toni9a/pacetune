...
const SPOTIFY_WINDOW_PADDING_MS = 15 * 60 * 1000;
const RUN_END_GRACE_MS = 60 * 1000;
...
function filterTracksOverlappingWindow(tracks, start, end) {
  return tracks.filter((track) => overlapSeconds(track.started_at, track.ended_at, start, end) > 0);
}
...
function assignTracksToSplits(splits, tracks) {
  for (const track of tracks) {
    ...
    for (const [index, split] of splits.entries()) {
      const effectiveSplitEnd = index === splits.length - 1
        ? new Date(split.end_time.getTime() + RUN_END_GRACE_MS)
        : split.end_time;
      const overlap = overlapSeconds(track.started_at, track.ended_at, split.start_time, effectiveSplitEnd);
    ...
    // Assign each song to one split only: whichever split owns the largest overlap.
    if (bestSplit && bestOverlap > 0) {
      bestSplit.songs.push(track);
    }
  }
}
...
export async function syncRunWindow({ stravaRefreshToken, spotifyRefreshToken, start, end }) {
  ...
  for (const run of runs) {
    ...
    const paddedStart = new Date(runStart.getTime() - SPOTIFY_WINDOW_PADDING_MS);
    const paddedEnd = new Date(runEnd.getTime() + SPOTIFY_WINDOW_PADDING_MS);
    const candidateTracks = await fetchSpotifyPlaysInWindow(spotifyAccess, paddedStart, paddedEnd);
    const effectiveRunEnd = new Date(runEnd.getTime() + RUN_END_GRACE_MS);
    const tracks = filterTracksOverlappingWindow(candidateTracks, runStart, effectiveRunEnd);
