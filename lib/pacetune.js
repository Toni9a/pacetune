...
function assignTracksToSplits(splits, tracks) {
  for (const track of tracks) {
    ...
    for (const split of splits) {
      const overlap = overlapSeconds(track.started_at, track.ended_at, split.start_time, split.end_time);
...
export async function syncRunWindow({ stravaRefreshToken, spotifyRefreshToken, start, end }) {
  ...
  for (const run of runs) {
    ...
    const tracks = filterTracksOverlappingWindow(candidateTracks, runStart, runEnd);
